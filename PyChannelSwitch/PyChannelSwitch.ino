...
// ADD HERE SUPPORT FOR RELAYS FOR VG/VDS
...
uint16_t currentChannelState = 0;
...
{
  uint16_t channelNumber = cmdMessenger.readBinArg<uint16_t>();  ////.readInt16Arg();//
  bool state = cmdMessenger.readBinArg<bool>();// readBoolArg();//
  ...
  if(isArgOk&&channelNumber>0&&channelNumber<=MAX_CHANNEL)
  {
    ...
    cmdMessenger.sendCmdArg(channelNumber);
    cmdMessenger.sendCmdArg(state);
    cmdMessenger.sendCmdEnd();
    //cmdMessenger.sendCmd(kAcknowledge,channelNumber);
  }else
  {
...
void OnMotorCommand()
{
  int16_t channelNumber = cmdMessenger.readBinArg<int16_t>();//.readInt16Arg();//.readBinArg<uint16_t>();
  int16_t channelSpeed = cmdMessenger.readBinArg<int16_t>();//.readInt16Arg();//.readBinArg<bool>();
  ...
  if(cmdMessenger.isArgOk()&&channelNumber>0&&channelNumber<=2)
  {
    ...
    cmdMessenger.sendCmdStart(kAcknowledge);
    cmdMessenger.sendCmdArg(channelNumber);
    cmdMessenger.sendCmdArg(channelSpeed);
    cmdMessenger.sendCmdEnd();
    //cmdMessenger.sendCmd(kAcknowledge, channelSpeed);
  }else
  {
...
void SwitchChannel(uint16_t number, uint16_t state)
{
  ...
  {
      SwitchChannel(currentChannel,0);
